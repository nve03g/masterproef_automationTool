...
#cond1 = osidj # in config file
#cond2 = sdof

# excel sheet/file inladen in memory vooraf alle tests -> sneller?
# in begin knn kiezen tussen controleren of aanpassen, andere methode om te werken
...
def max_char(filename, sheetname, column_name, max_chars):
    ...
    # -> kunnen we dit via config file ingeven? zeggen wat kolomnamen zijn en waar de headers te vinden zijn
    ...
    for row, cel in enumerate(ws[column_letter][3:], start=4): # start at row 4
        if isinstance(cel.value, str) and len(cel.value) > max_chars:
            errors.append(f"Row {row}: '{cel.value}' is too long ({len(cel.value)} > {max_chars})")
...
def alarm_exists(filename, sheetname, alarm_column_name, row_nr):
    wb = xl.load_workbook(filename) # load Excel file
    ws = wb[sheetname] # access the right sheet
    ...
    column_letter = headers[alarm_column_name]  # fetch column letter ('A', 'B' ...)

    # non existing -> empty, "reserved" or "spare"
    # Nellie: OpenPyXL ondersteunt geen tekststijlen zoals doorstrepen, daarvoor moet je XLWings gebruiken
    cell = ws[f"{column_letter}{row_nr}"] # access specific cell to check if alarm exists in this row
    if cell.value in [None, "", "reserved", "spare"]: # wat met auxX en nederlands?, knn we dit via config doen?
        # print(cell.coordinate) # print coördinaten van non-existing alarms
        return False
    return True
...
def empty_with_correction(filename, sheetname, column_name, alarm_column_name, must_be_empty=True):
    ...
    if correction_count > 0:
        ...
        print(f"Corrected file saved as '{f"{filename}_corrected.xlsx"}' with {correction_count} changes.") # filename opsplitsen in effectieve naam en extension
    else:
...
# max_char(filename="voorbeeld_alarmlijst.xlsx", sheetname="Alarmlist", column_name="Dutch translation", max_chars=100)

# empty("voorbeeld_alarmlijst.xlsx", "Alarmlist", "Class", "Alarmtext machine constructor (German)", must_be_empty=False, condition=[cond1, cond2, ...]) # conditions vullen vanuit config file
...
# file_type(filename="voorbeeld_alarmlijst.xlsx", sheetname="Alarmlist", column_name="Picture", alarm_column_name="Alarmtext machine constructor (German)", file_extension=".pdf")
